...
void fat_file_unlink(fat_file file, fat_file parent) {
    u32 cur_cluster = 0, next_cluster = 0;
    cur_cluster = file->start_cluster;

    while (!fat_table_is_EOC(file->table, cur_cluster)) {
        next_cluster = fat_table_get_next_cluster(file->table, cur_cluster);
        fat_table_set_next_cluster(file->table, cur_cluster, FAT_CLUSTER_FREE);
    ...
    while (bytes_remaining > 0 && !fat_table_is_EOC(file->table, cluster)) {
        ...
        if (bytes_remaining > 0) {
            if (!fat_table_is_EOC(file->table, cluster)) {
                u32 new_cluster = fat_table_get_next_free_cluster(file->table);
                fat_table_set_next_cluster(file->table, cluster, new_cluster);
                fat_table_set_next_cluster(file->table, new_cluster,
                                           FAT_CLUSTER_END_OF_CHAIN);
            }
            cluster = fat_table_get_next_cluster(file->table, cluster);
            if (errno != 0) {
