...
static void fat_fuse_create_fslog(void);
...
static void fat_fuse_log_activity(char *operation_type, fat_file target_file) {
    ...
    strcat(buf, "\n");
    // Add the complete string 'buf' to the target_file
    fat_volume vol = get_fat_volume();
    fat_file file = fat_tree_search(vol->file_tree, "/fs.log");
    fat_tree_node file_node = fat_tree_node_search(vol->file_tree, "/fs.log");
...
static int fat_fuse_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
    ...
    child = children;
    while (*child != NULL) {
        // if child = /fs.log then ignore.
        if (fat_file_cmp_path(*child, "/fs.log")) {
...
static void fat_fuse_create_fslog(void) {
    fat_volume vol = get_fat_volume();
    fat_file fslog, parent;
    fat_tree_node node = fat_tree_node_search(vol->file_tree, "/fs.log");
    if (node == NULL) {
        fat_fuse_mknod("/fs.log", 1, 1);
        node = fat_tree_node_search(vol->file_tree, "/fs.log");
        fslog = fat_tree_get_file(node);
        parent = fat_tree_get_parent(node);

        // Set 'fslog' attributes to hide from external File Systems
